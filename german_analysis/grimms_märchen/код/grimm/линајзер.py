from grimm.токенизер import ТокенСпејс, ТокенЗаКрај, ТокенСам, ТокенРеч, ТокенЗатворениНаводник, ТокенОтворениНаводник
КРАЈ = ТокенЗаКрај('', -1)
...
def јел_наводник(т):
    return отворени_наводник(т) or затворени_наводник(т)
    ...
    def линија(бре, т):
        ...
        if спејс(т):
            ...
            if крај(т):
                ...
                return т
        while True:
            if јел_наводник(т) and not бре.директан_говор:
                if л != "":
                    бре.додај(л)
                т = next(бре.и, КРАЈ)
                бре.директан_говор = True
                return т
            elif јел_наводник(т) and бре.директан_говор:
                бре.додај(л)
                т = next(бре.и, КРАЈ)
                бре.директан_говор = False
                return т
