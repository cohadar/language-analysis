...
class Линајзер():
    ...
    def линија(бре, т):
        ...
        while True:
            if је_токен_за_крај(т):
                ...
                т = бре.додај_линију(л, True)
                бре.директан_говор = False
                return т
            elif је_нова_линија(т):
                т = бре.додај_линију(л, бре.директан_говор, форсирај=True)
                return т
            elif крај_реченице(т) or крај_дела_реченице(т) or је_нова_линија(т):
                л.append(т)
                т = бре.додај_линију(л, бре.директан_говор)
                return т
            else:
    ...
    def додај_линију(бре, л, цитирај, форсирај=False):
        if not л:
