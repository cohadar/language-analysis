...
class Линајзер():
        ...
        while True:
            ...
            if је_затворени_наводник(т):
                if not бре.директан_говор:
                    ...
                    pass
                т = бре.додај_линију(л, True, нова_линија=True)
                бре.директан_говор = False
                return т
            if крај_реченице(т) or крај_дела_реченице(т):
                if је_затворени_наводник(бре.и.следеће):
                    л.append(т)
                    т = бре.додај_линију(л, бре.директан_говор, нова_линија=True)
    ...
    def додај_линију(бре, л, цитирај, нова_линија=False):
        л = трим(л)
        if not л:
            return next(бре.и, ТОКЕН_ЗА_КРАЈ)
        л2 = []
