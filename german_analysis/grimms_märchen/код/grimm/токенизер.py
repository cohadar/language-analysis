...
class Токенизер():
    ...
    # def наводник(бре, к):
    #     if бре.и.претходно in [' ', '\n', -1, ':'] and СЛОВА.fullmatch(бре.и.следеће):
    #         бре.токени.append(ТокенОтворениНаводник('„', бре.и.линија))
    #     elif бре.и.претходно in [',', ';', '.', '!', '?'] and бре.и.следеће in [' ', '\n', -1]:
    #         бре.токени.append(ТокенЗатворениНаводник('“', бре.и.линија))
    #     elif СЛОВА.fullmatch(бре.и.претходно) and бре.и.следеће in [' ', '\n', -1, ';']:
    #         бре.токени.append(ТокенЗатворениНаводник('“', бре.и.линија))
    #     elif бре.и.претходно == ' ' and бре.и.следеће == ' ' and бре.токени[-2].текст in [',', ';', '.', 

    #     else:
    #         if к == "'":
    #             бре.токени.append(ТокенСам(к, бре.и.линија))
    #         else:
    #             досад = ''.join([т.текст for т in бре.токени])
    #             raise Exception(f'Лош цитат: ({к}), линија={бре.и.линија}, текст={досад[-100:]}')
    #     return next(бре.и, КРАЈ)
